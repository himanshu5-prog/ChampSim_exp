...
void O3_CPU::last_branch_result(uint64_t ip, uint64_t branch_target, uint8_t taken, uint8_t branch_type){
   ...
   auto x = localHistReg.to_ullong();
    ...
    if ((::currentPred && !taken) || (!(::currentPred) && taken)){
        // Misprediction
        if (takeLocal(::predCounter)){
            // Local predictor was used. Lean towards global predictor
            ::predCounter = incrementCounter(::predCounter);
        } else {
            //Lean towards local predictor
            ::predCounter = decrementCounter(::predCounter);
        }
    } else {
        // Correct prediction
